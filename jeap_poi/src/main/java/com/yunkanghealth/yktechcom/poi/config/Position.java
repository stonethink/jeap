...
public class Position {
	...
	public void setPositionStr(String positionStr) {
		...
		if (JeapStringUtil.isEmpty(positionStr)) {
			setRow(0);
			setColumn(0);
			return;
		}
		...
		rowStr = JeapStringUtil.getNumbers(str);
		colStr = JeapStringUtil.splitNotNumber(str);
		colNo = colStr2ColNo(colStr);
		
		if (!JeapStringUtil.isEmpty(rowStr)) {
			setRow(Integer.parseInt(rowStr));
		}
		if (!JeapStringUtil.isEmpty(colStr)) {
			setColumnStr(colStr);
			setColumn(colNo);
		}
	}
