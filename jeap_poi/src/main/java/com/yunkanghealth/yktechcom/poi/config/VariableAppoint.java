...
public class VariableAppoint {
	...
	private VariableDefine variableDefine;
	//private VariableDefine defaultObjDefine;
	...
	/*
		public VariableAppoint(Map<String, VariableDefine> variablesMap,
				VariableDefine defaultObjDefine) {
			super();
			setVariablesMap(variablesMap);
			setDefaultObjDefine(defaultObjDefine);
		}
	*/
	...
	private Object wrapDataValue(Object varValue) {
		Object value = null;
		if (JeapStringUtil.isEmpty(preVarStr)
				&& JeapStringUtil.isEmpty(postVarStr)) {
			value = varValue;
		} else {
			...
			if (!JeapStringUtil.isEmpty(postVarStr)) {
	...
	/**
	public void putValue(Map<String, Object> impData) {
		putValue(this.value, impData);
	}

	public void putValue(Object varValue, Map<String, Object> impData) {
		boolean isNewRecord = false;
		putValue(varValue, impData, isNewRecord);
	}

	public void putValue(Map<String, Object> impData, boolean isNewRecord) {
		putValue(this.value, impData, isNewRecord);
	}
	...
	public void putValue(Object varValue, Map<String, Object> impData,
		...
		Object value = unWrapDataValue(varValue);
		if (variableDefine != null) {
			variableDefine.putValue(value, impData, isNewRecord);
		}
		log.debug("varName=" + this.dumpStr() + "; varValue=" + varValue);
	}
	**/
	...
	public static VariableDefine parserVaribaleDefine(String configStr,
		...
		List<String> classList = JeapStringUtil
		...
		List<String> varCfgList = JeapStringUtil.splitByFlag(configStr, "$[",
				"]");
		...
		if (varDefine != null) {
			/**			
						if (isProperty) {
							varDefine.setVarType(VariableDefine.VAR_TYPE_PROPERTY);
						}else{
	...
	private static VariableDefine parserVarStrNoCalssName(
		...
		if (JeapStringUtil.isEmpty(varStr)) {
			return varDefine;
		}
		...
		if (varDefine != null && !JeapStringUtil.isEmpty(tranMapStr)) {
			parserTranMap(varDefine, tranMapStr);
		}

		if (varDefine != null && !JeapStringUtil.isEmpty(groupTypeStr)) {
	...
	private static VariableDefine parserVarNameStr(
		...
		if (JeapStringUtil.isEmpty(varNameStr)) {
			return varDefine;
		}
	...
	private static VariableDefine parserVarNameSplitByColon(
		...
		for (int idx = 0; idx < splitNameNum; idx++) {
			...
			if (JeapStringUtil.isEmpty(oneNameStr)) {
				// do nothing
			} else {
	...
	/*
		public VariableDefine getDefaultObjDefine() {
			return defaultObjDefine;
		}

		public void setDefaultObjDefine(VariableDefine defaultObjDefine) {
			this.defaultObjDefine = defaultObjDefine;
		}
	*/
