...
/****
	public int getDefineRowNum() {
		int rowNum = 0;
		int begDefineRow;
		int endDefineRow;

		if (rowDefineList == null || rowDefineList.size() == 0) {
			rowNum = 0;
		} else {
			begDefineRow = rowDefineList.get(0).getPosition().getBegRow();
			endDefineRow = rowDefineList.get(rowDefineList.size() - 1)
					.getPosition().getBegRow();
			rowNum = endDefineRow - begDefineRow + 1;
		}
		return rowNum;
	}
****/
