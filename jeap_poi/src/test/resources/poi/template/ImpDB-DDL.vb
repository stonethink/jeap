...
' _TBL_   区域填充类型    起始单元格  结束单元格  数据非空检查行/列   标题起始行(相对起始位)  标题行数    数据起始行(相对起始位)  数据结束行(相对结束位)  模板行数    缺省数据对象名
Dim tplAreaConfigs(1 To 5, 10) As Variant
Dim AreaConfigs(1 To 5, 10) As Variant

Private Type AreaConfig
    areaName        As String
    fillModel       As String
    beginCell       As String
    endCell         As String
    dataBeginRow    As Integer
    dataEndRow      As Integer
    tplRowNum       As Integer
End Type
...
Private Sub ImpOneWorkBook(wbWorkbook As Workbook)
    ...
    Dim sCurrPath As String: sCurrPath = ThisWorkbook.Path
    sOutputFileName = sCurrPath & "\" & Left(sTmpFileName, Application.Find(".", sTmpFileName) - 1) & ".sql"
...
Private Sub DumpTblSheets(wbWorkbook As Workbook)
    Dim iSheetSeq As Integer: iSheetSeq = 0
    
    Dim bIsDump As Boolean
    Dim wsWorkSheet As Worksheet
    For Each wsWorkSheet In wbWorkbook.Worksheets
        bIsDump = isDumpSheet(wsWorkSheet)
        If bIsDump = True Then
            DumpOneTblSheet wsWorkSheet
        End If
    Next
End Sub

Private Sub DumpOneTblSheet(wsWorkSheet As Worksheet)
    InitSheetLogHeader
